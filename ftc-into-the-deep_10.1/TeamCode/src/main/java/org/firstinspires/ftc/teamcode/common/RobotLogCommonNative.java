...
// This version of RobotLogCommon uses the native Java logger
// to write time-stamped log files to the file system on the
// RobotController; the directory is sdcard/FIRST/TeamData/logs.
// This logger has advantages in that each log file is separate
// (it does not overwrite log files the way the the FTC does
// with matchlogs). This logger also omits the sometimes
// voluminous log records from the FTC SDK itself. But this
// native logger has a disadvantage in the fact that the users
// must manually delete old logs from the logs directory.

// To switch to this logger take the following actions *outside*
// Android Studio on the two files in the current project under
// the directory ...\TeamCode\src\main\java\org\firstinspires\ftc\teamcode\common:
// RobotLogCommon.java (assuming this is the pass-through logger
// currently in use) and RobotLogCommonNative.java (this file).
// 1. Rename RobotLogCommon.java to RobotLogCommonFTC.java and
//    then inside the file change the class name and the class
//    reference in the TAG to RobotLogCommonFTC.
// 2. Rename this logger to RobotLogCommon.java and then inside
//    the file change the class name and the class
//    reference in the TAG to RobotLogCommon.

// Logging class that enqueues log records and uses the standard
// Java logger to write them out to files in the background.
...
// in which Autonomous and/or TeleOp are run multiple times without
...
public class RobotLogCommonNative {

    private static final String TAG = RobotLogCommonNative.class.getSimpleName();
    ...
    private static final CommonLogLevel DEFAULT_LEVEL = CommonLogLevel.d;
    ...
    public static synchronized OpenStatus initialize(LogIdentifier pIdentifier, String pLogDirPath) {
        ...
        try {
            ...
            logger.setLevel(DEFAULT_LEVEL.getJavaLogLevel());
            ...
            RobotLog.dd(TAG, "Error in logger initialization; logging is disabled");
        }
    ...
    public static synchronized void setMostDetailedLogLevel(final CommonLogLevel pLogLevel) {
        ...
        currentLogData.logger.setLevel(pLogLevel.getJavaLogLevel());
        currentLogData.logLevel = pLogLevel;
    }

    public static synchronized boolean isLoggable(CommonLogLevel pLogLevel) {
        return currentLogIdentifier != LogIdentifier.NONE && currentLogData.logger.isLoggable(pLogLevel.getJavaLogLevel());
    }

    public static synchronized CommonLogLevel getMostDetailedLogLevel() {
        if (currentLogIdentifier == LogIdentifier.NONE)
            return CommonLogLevel.off;
        return currentLogData.logLevel;
    }
    ...
    private static synchronized void enqueueLogEntry(Level pLevel, String pLogString) {
        if (currentLogIdentifier == LogIdentifier.NONE || currentLogData.logLevel == CommonLogLevel.off)
...
02-08 15:55:08.995   997  1378 D FTCRobotLog: Error java.lang.InterruptedException
    ...
    private static class LogData {
        ...
        public CommonLogLevel logLevel = DEFAULT_LEVEL;
